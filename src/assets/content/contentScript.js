...
// Utility: Break lines longer than 150 characters
function breakLongLines(text) {
  // Split by newline, then break any line > 150 chars
  return text
    .split("\n")
    .map((line) => {
      ...
      let result = "";
      for (let i = 0; i < line.length; i += 150) {
        result += line.slice(i, i + 150) + "\n";
      }
      return result.trim();
    })
    .join("\n");
  ...
  favBtn.addEventListener("click", () => {
    ...
    if (storedArticels.length > 0) {
      if (!storedArticels.some((art) => art.id === short_article.id)) {
        ...
        const removed = storedArticels.filter(
          (art) => art.id.toString() !== short_article.id.toString()
        );
...
const createToggleBtn = (block, short_article, isScrolled = false) => {
  const hideBtn = createButton("Less", () => {
    ...
    hideBtn.remove();
    block.parentNode.insertBefore(previewDiv, block);
    block.parentNode.insertBefore(showBtn, block.nextSibling);
  });

  // Show truncated preview of first 150 characters
  ...
  // Container for See More and icon, placed below preview
  ...
  if (!isScrolled) {
    ...
    const getShowBtnElement = document.querySelector(
      `.show_btn_${short_article.id}`
    );
  ...
  try {
    const assistantMessages = Array.from(
      document.querySelectorAll(
        '[data-message-author-role="assistant"]:not([data-processed])'
      )
    );
    const userMessages = Array.from(
      document.querySelectorAll(".whitespace-pre-wrap:not([data-processed])")
    );
    ...
    const processAssistantBlock = (block, index) => {
      ...
      const preview =
        words.length > 5 ? words.slice(0, 5).join(" ") + "..." : originalText;
        ...
        if (delete_btn) {
          ...
          delete_btn.addEventListener("click", () => {
            if (storedArticels.length > 0) {
              if (
                storedArticels.some(
                  (art) => art.id.toString() === articleId.toString()
                )
              ) {
                const removed = storedArticels.filter(
                  (art) => art.id.toString() !== articleId.toString()
...
function renderTablePage(page) {
  ...
  const pageItems = storedArticels.slice(start, end).sort((a, b) => {
    return new Date(a.savedDate) < new Date(b.savedDate)
      ? 1
      : a.savedDate > new Date(b.savedDate)
      ? -1
      : 0;
  });
    ...
    for (const mutation of mutationsList) {
      if (mutation.type === "attributes") {
        ...
        if (oldValue === newValue) {
          ...
          console.log(
            `Attribute "${mutation.attributeName}" changed from "${oldValue}" to "${newValue}"`
          );
        }
  ...
  const observerBody = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      ...
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === 1 && node.nodeName === "ARTICLE") {
